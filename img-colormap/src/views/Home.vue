<template>
  <div id="colormap">
    <select v-model="currentCM" class="colormap-select" id="colormap-select" @change="initColors">
      <option v-for="(cm, index) in colormaps" :key="index" :value="cm">{{cm}}</option>
    </select>
    ...
    <input type="button" value="随机" @click="generateImageData" />
    <div class="to-gray-w">
      <div class="scream">
        <img id="scream" alt="Image preview..." :src="imgUrl" />
      </div>
      <canvas id="canvas" width="336px;" height="256px;">your browser does not support canvas!</canvas>
    </div>
...
export default {
  ...
  methods: {
    ...
    //初始化colors
    initColors() {
      var e = document.getElementById("colormap-select");
    ...
    getOriginImageData() {
      this.loadImgCallback().then(async () => {
        ...
        originImageData = ctx.getImageData(
          0,
          0,
    ...
    showImageByData(imageData) {
      ...
      console.log("showImageByData over.");
    },
    //保存文件到本地
